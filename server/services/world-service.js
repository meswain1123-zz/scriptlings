...
function getSense(respond, scriptling, user) {
  ...
  console.log(world);
  db.senseResources(sensedResources, scriptling, myEnv.worldID, world.scriptlingFormula.senseRange);

  function sensedResources(resources) {
    console.log(resources);
    senseObj.resources = resources.map(element => {
      element.location[0] -= user.home[0];
      element.location[1] -= user.home[1];
      return element;
    });
